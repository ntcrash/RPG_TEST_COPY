...
## 07/13/2026 - v3.2.2
### 🧹 Consolidate completed roadmap history
Moved the completed pygame → Arcade migration detail from `ROADMAP.md` into this changelog so the roadmap remains a concise living backlog. The roadmap now contains only its instructions and empty priority sections; no actionable items remain.
- **Verification**: `git diff --check` clean; no runtime code changed beyond the patch version metadata.
...
**Multiplayer players can now create an account and link their characters to it, so they log in once and choose which of *their* characters to bring online — while single-player is completely unchanged.** An **account** is just a login that indexes existing character saves; it does not duplicate or own character data. Everything here is **purely additive**: nothing in the single-player import path references the account layer, single-player still discovers characters by scanning `Characters/`, and accounts are used only on the (opt-in) multiplayer path. The whole layer is **standard-library only** (`hashlib`/`hmac`/`json`/`os`/`re`/`time` — no pygame/arcade) so it imports and unit-tests headlessly with no display.
- **New pure module `Code/account_system.py`** — the dependency-free core. `AccountManager` does CRUD over one JSON file per account under an `Accounts/` directory (path passed in, so it's a pure function of that directory + its arguments):
  - **Usernames** are validated (3–24 chars, `[A-Za-z0-9_.-]` only), normalized case-insensitively (so `Joe` == `joe`), and kept filesystem-safe so `<username>.json` never needs escaping and can't traverse directories.
  - **Passwords are never stored in plaintext.** Each account keeps a random per-account salt and a **PBKDF2-HMAC-SHA256** hash (`PBKDF2_ITERATIONS = 200_000`, stored per-account so it can be raised later without invalidating existing accounts); `verify_password` compares in constant time via `hmac.compare_digest`. A `MIN_PASSWORD_LENGTH = 4` floor guards casual local misuse.
  - **Character membership** — `add_character`/`remove_character`/`list_characters` register character *names* under the account (idempotent, case-insensitive, capped at `MAX_CHARACTERS_PER_ACCOUNT = 32`). Accounts **index** existing `Characters/*.json` saves — `remove_character` only unlinks the reference, it never deletes the underlying save.
  - **`public_view(record)`** returns a secret-free copy (username + linked characters, no salt/hash) so an account can be surfaced to the game/UI or the network without leaking credentials, and **`authenticate_or_create(manager, username, password)`** is a one-call convenience for a single multiplayer "Account" prompt — logs in if the account exists (raising on a wrong password), otherwise creates it. `Account_Version = 1` anchors the on-disk schema for future changes.
- **Opt-in wiring into the multiplayer layer (`Code/multiplayer.py`)** — `MultiplayerSession` gained an optional `account` field carrying the public account view (`None` in accountless/guest play). A new `account_from_env()` reads `MEGITECH_ACCOUNT` / `MEGITECH_ACCOUNT_PASSWORD` (and optional `MEGITECH_ACCOUNTS_DIR`); when both credentials are present it logs the player in — creating the account on first use — and returns the public view, and `session_from_env()` attaches it to the session. A bad or absent account **degrades gracefully to guest multiplayer** rather than crashing the join, and the display name still resolves explicit `MEGITECH_PLAYER_NAME` → account username → `"Adventurer"`. Accountless multiplayer and single-player are both unchanged.
- **New directory** — added `Accounts/` (with `.gitkeep`) to hold the per-account JSON files, mirroring `Characters/`/`SaveProgression/`.
- **Tests** — added `tests/test_account_system.py` (35 tests, headless, standard-library only): username/password validation and normalization, password hashing/verify round-trips + salt independence + not-plaintext + corrupt-record handling, full `AccountManager` create/authenticate/list CRUD, character membership (add/remove/idempotency/cap/missing-account errors/don't-touch-others), `public_view` secret-stripping + list-copy, and the `MultiplayerSession` env path (account created-then-logged-in, disabled without creds, wrong-password degrades to `None`/guest, explicit name overrides account username, account attached with default name).
- **Verification**: `py_compile` clean on all changed modules; full suite **313 → 348** green (6 skipped when arcade/display absent). Confirmed nothing in the single-player import path references the account layer. *(Ruff was unavailable in the automation sandbox this run; the new code follows the existing style and adds no new imports beyond the standard library — the 42-finding Ruff cleanup remains tracked in the ToDo.)*
- **Result**: roadmap **P6 #41 (account creation) is done** — this closes out the entire **P6 multiplayer** backlog. In-game rendering of remote players on the board (built on the v3.0.1 transport) remains the natural next follow-up.

## 07/11/2026 - v3.0.1
### 🌐 Multiplayer — client/server networking foundation (roadmap P6 #40)
**Adds the server component and a client networking layer so multiple players can share a world, while single-player is completely unchanged.** This is the transport/presence foundation of multiplayer: each connected client streams its own state (position, level, hp, …) and the server relays every player's state, plus join/leave and chat, to everyone else. World and combat logic still run client-side exactly as in single-player — the server is a lightweight *relay/presence* server, not a physics authority. Everything here is **purely additive**: no single-player module imports any of it, and the default `python main.py` path never opens a socket, so single-player behaviour cannot change. The whole layer is **standard-library only** (`socket`/`threading`/`socketserver`/`json`/`struct`) — no pygame/arcade — so the server runs on a headless box and the code unit-tests without a display.
- **New pure wire protocol `Code/network_protocol.py`** — the dependency-free core. Defines the message vocabulary (`MsgType`: `join`/`join_ack`/`join_reject`/`state`/`player_state`/`player_joined`/`player_left`/`chat`/`chat_broadcast`/`ping`/`pong`/`leave`/`error`) and a **length-prefixed JSON framing** (`4-byte big-endian length` + UTF-8 JSON body) so a TCP byte stream can be split back into whole messages regardless of fragmentation. `encode_message`/`decode_body` are pure; `FrameDecoder` is a tiny buffering state machine that reassembles partial/coalesced frames. Guards against malformed input: unknown message types, non-JSON payloads, non-object payloads, and an 8 MiB `MAX_FRAME_BYTES` cap so a bogus length prefix can't exhaust memory. `PROTOCOL_VERSION = 1`, `DEFAULT_PORT = 50007`.
- **New server `Code/game_server.py`** — `GameServer`, a threaded (`ThreadingTCPServer`) presence/relay server. Assigns each client a `player_id`, sends a `join_ack` carrying every existing player's current state, then broadcasts `player_joined`/`player_state`/`player_left`/`chat_broadcast` to the rest. Thread-safe registry under an `RLock`; `start()` spawns a daemon thread and returns the bound port (`port=0` binds an ephemeral port for tests), `stop()` tears everything down. Rejects clients on a protocol-version mismatch or a pre-join message. Includes `run_server()` for a blocking standalone process.
- **New client `Code/network_client.py`** — `NetworkClient` wraps the socket and runs a background receive thread that keeps a thread-safe mirror of every *other* player's `{name, state}` (`remote_players()`). `connect()` performs the join handshake and blocks until acked (or rejected/timed-out — never hangs); `send_state()`/`send_chat()`/`ping()` publish outbound; an optional `on_event(type, payload)` callback fires for chat/pong/presence. A failed connect surfaces as a return value, never an exception that could reach the game.
- **New opt-in session helper `Code/multiplayer.py`** — `MultiplayerSession`, a display-free façade the future rendering step can poll each frame (`publish(x=…, y=…, level=…)`, `others()`, `chat()`). Multiplayer is **opt-in only**: `session_from_env()` returns a session **only** when `MEGITECH_MULTIPLAYER=1` (with `MEGITECH_SERVER=host:port` and `MEGITECH_PLAYER_NAME`), otherwise `None` so the game stays single-player. `MultiplayerSession.start()` swallows connection failures into `last_error` and returns `False` — a server being down can never crash the game.
- **New standalone launcher `server_app.py`** — the server counterpart to the Arcade client's `arcade_app.py`. `python server_app.py [--host H] [--port P]` runs a blocking, headless server. Clients join with `MEGITECH_MULTIPLAYER=1 MEGITECH_SERVER=<host>:<port> python main.py`.
- **Tests** — added `tests/test_network.py` (26 tests, headless, standard-library sockets only): protocol encode/decode round-trips and rejection paths, `FrameDecoder` reassembly (byte-at-a-time fragmentation, multiple frames per chunk, partial-header buffering, trailing-partial retention, frame-length cap), server lifecycle (ephemeral bind, double-start guard), and full **loopback integration** against a real `GameServer` on `127.0.0.1:0` — single/dual client join, protocol-mismatch rejection, two-way state sync, existing-players-in-ack, leave-removes-player, chat broadcast, ping/pong, plus the high-level `MultiplayerSession` end-to-end and env-gating.
- **Verification**: `py_compile` clean on all new modules; the five new modules + test file are **Ruff-clean** (zero new findings); full suite **287 → 313** green (6 skipped when arcade/display absent). Confirmed nothing in the single-player import path references the new modules.
- **Result**: roadmap **P6 #40 (server/client setup + version to v3.0.1) is done.** Remaining P6 work: **#41 — account creation** (accounts + characters-under-account for multiplayer). In-game rendering of remote players on the board is the natural next follow-up on top of this transport layer.

## 07/11/2026 - v2.12.0
### 🐾 Helper pets — combat companions earned from bosses or bought (roadmap P5 #32)
**Players can now recruit a companion pet that fights alongside them, either as loot from beating a boss or by buying it in the shop.** Once a pet is owned and set active, it performs an automatic assist every combat round — striking the enemy, mending the player, or both — so party progression gains a whole new axis beyond gear and spells.
- **New pure module `Code/pet_system.py`** — a display-free, JSON-friendly core so every consumer shares one source of truth and it unit-tests headlessly:
  - **`PET_CATALOG`** of five companions: **Dire Wolf** (attack, Lv 1, 800cr), **Healing Sprite** (heal, Lv 2, 1200cr), **Ember Drake** (attack, Lv 5, 2400cr), **Phoenix** (leech — damage + half-heal, Lv 8, 4000cr), and the **Void Shade** (attack, Lv 10) — a prestige pet that is **boss-drop only, never sold**. Each entry carries its assist type, base power, display verb, unlock level, price, and source (`shop`/`boss`/`both`).
  - **`compute_pet_assist(pet, player_level)`** — the per-turn effect. Amount is `power + level // 2` (a modest, deterministic scaling so a companion stays useful as the hero grows without outshining them). `attack` damages the enemy, `heal` restores player HP, `leech` damages the enemy and returns half as healing. Pure/no-randomness → predictable combat and trivially testable.
  - **Eligibility helpers** — `shop_pets(player_level=None)` (buyable, optionally level-filtered) and `boss_droppable_pets(boss_level, owned)` (droppable, excludes owned + level-locked).
  - **`roll_boss_pet_drop(char_data, boss_level, rng=random)`** — a `BOSS_DROP_CHANCE_PERCENT = 35` roll that returns a random eligible, not-yet-owned pet id (or `None`). `rng` is injectable so tests pin the outcome; it does not mutate the save.
  - **`PetManager`** — thin wrapper over `character_manager.character_data` for ownership: `owned_pet_ids`/`owns`/`active_pet` (ignores stale/unowned ids), `own_pet` (first pet auto-activates), `set_active` (`""` clears), and a full `buy_pet` returning `purchased` / `already_owned` / `not_for_sale` / `level_locked` / `insufficient_funds` / `no_character`.
- **Combat assist wired in** — `EnhancedCombatManager.pet_assist_turn()` reads the active pet and applies its assist. It runs at the start of the enemy-turn transition (right before `enemy_turn`), so if the pet's strike finishes the enemy, the existing HP≤0 guard skips the enemy's attack and the next update resolves the victory. Heals are clamped to max HP; floating combat text + a log line report each assist.
- **Boss loot** — `EnhancedCombatIntegration.award_boss_pet(boss_level)` rolls a drop on every boss victory (right after the level is completed), records ownership (auto-active if it's the player's first pet), and announces the new companion in the combat log.
- **Shop integration** — `StoreManager` now lists the buyable pets (sourced straight from `shop_pets()`, so store and catalog never drift) as `item_type="pet"` rows; `StoreItem` gained an optional `pet_id`. `attempt_purchase` routes pet rows through `PetManager.buy_pet` (its own affordability/level/already-owned checks; ownership recorded on the save, **not** stuffed into the item Inventory).
- **Persistence** — bumped `SAVE_SCHEMA_VERSION` **1 → 2**: `Code/save_migration.py` now backfills and validates `Pets` (list of owned ids) and `Active_Pet` (active id, `""` = none); `create_sample_character` seeds both. Old saves are healed to the new shape on first load with no data loss.
- **Tests** — added `tests/test_pet_system.py` (45 tests, headless via the `Code.gfx` shim): catalog integrity + unique ids, shop/boss eligibility & level filtering, assist math per type + level scaling + determinism, seeded boss-drop (hit/miss/empty-pool), `PetManager` ownership & the full purchase matrix, save-migration backfill/validation, store listing + buy-through, and combat assist application (attack damages, heal restores without overhealing, leech does both, no-pet no-op, finishing blow). Updated one v2.3.0 migration fixture for the new container field.
- **Verification**: `py_compile` clean on all changed modules; the new module + test file are Ruff-clean and add **zero** new findings (the 2 pre-existing `store_system.py` findings — unused `import random`, a `B007` loop var — predate this change and remain tracked in the ToDo); full suite **242 → 287** green (6 skipped when arcade/display absent).
- **Result**: roadmap **P5 #32 (Add helper pets) is done** — this closes out the entire P5 backlog.

## 07/11/2026 - v2.11.0
### ✨ More spells — two new tiers per aspect, level-gated (roadmap P5 #33)
**Every magic aspect now has five spells instead of three, with the two new tiers unlocking as the character levels up.** Spellcasters previously hit their ceiling at level 5 (the "advanced" spell); there was nothing new to earn afterward. This adds a **master** tier (unlocks at level 8) and an **ultimate** tier (unlocks at level 12) to all six aspects, extending magical progression well into the late game.
- **New spells (one master + one ultimate per aspect)** — Fire: **Meteor** / **Supernova**; Water: **Absolute Zero** / **Tsunami**; Dream (lightning): **Tempest** / **Cataclysm**; Earth: **Tectonic Shift** / **World Ender**; Life: **Divine Restoration** / **Resurrection** (both heals); Void: **Void Rift** / **Oblivion** (Oblivion is a life-drain). Each carries a status effect consistent with its aspect (burn/freeze/stun/knockdown/curse) and costs more mana than any earlier tier, so they're a meaningful power step, not a free upgrade.
- **Data-driven level gating** — added a module-level `SPELL_UNLOCK_LEVELS = (1, 3, 5, 8, 12)` (in both `Code/enhanced_combat_system.py` and the legacy `Code/combat_system.py`) and rewrote `SpellManager.get_spells_for_aspect` to zip each aspect's spell list against those thresholds. The old hard-coded `if level >= 1/3/5` ladder only ever returned three spells; the new form returns however many tiers are unlocked and adapts automatically if more tiers are ever added. The empty-list fallback (always return at least the basic spell) is preserved, as is the unknown-aspect → fire fallback.
- **Adaptive spell-menu layout** — the enhanced combat spell menu (`Code/enhanced_combat_system.py` draw) previously assumed a worst case of 3 spells (fixed 45px row pitch); a 4–5 spell list would have run its box and instruction bar down into the combat log (top y=455). The menu now computes an adaptive `row_pitch` (default 45px, compressed toward a 34px floor only when needed) so a full five-spell list always sits clear of the log. `menu_height`, the row loop, and the instruction-bar position all derive from that pitch.
- **Parity kept** — the two `SpellManager` copies (enhanced = live UI, legacy = retained/tested) get identical libraries and gating so tests can pin them against each other.
- **Tests** — added `tests/test_more_spells.py` (15 tests, headless via the `Code.gfx` shim): five tiers per aspect, the new spells present, attribute validity, new-tier mana pricing above all earlier tiers, the full unlock threshold table (1→1, 3→2, 5→3, 8→4, 12→5 spells), never-empty and lowest-tier-first ordering, unknown-aspect fallback, legacy/enhanced library + gating parity, and the adaptive-layout math proving a 5-spell menu clears the combat log.
- **Verification**: `py_compile` clean on all modules; the new test file is Ruff-clean and introduces **zero** new findings (the 14 pre-existing findings in the two combat modules — star-import F405s, unused `enemy_stats`/`title_color` — predate this change and remain tracked in the ToDo); full suite **227 → 242** green (6 skipped when arcade/display absent).
- **Result**: roadmap **P5 #33 (Add more spells) is done.**

## 07/11/2026 - v2.10.0
### 🛤️ Structured Paths — clear walking routes to every important area (roadmap P5 #31)
**The overworld now has obvious dirt routes leading the player toward each key destination.** The map's important interactables — the boss dungeon (world centre), the shop (top-right), and the rest area (bottom-right) — sat at fixed spots but nothing on the ground pointed the way, so the world read as a trackless field of grass. Structured Paths carves clear routes so the player always has a road to follow.
- **New route helper** — added `EnhancedTileMap.carve_landmark_paths(lines)` (`Code/tile_map.py`): a pure, deterministic classmethod that draws an L-shaped (Manhattan) dirt-path route from the central hub (boss dungeon) out to the player spawn, the shop, and the rest area. Where a route crosses an existing path or another route the cell becomes an intersection (`+`); plain segments are dirt path (`p`). Length-preserving and display-free, so it unit-tests headlessly.
- **Landmark locator** — added `EnhancedTileMap.landmark_tiles(cols, rows)` returning the `(col, row)` tile of the hub / spawn / shop / rest for a given grid, derived from the grid size in the **same pixel terms** the game uses to place those objects (`EnhancedGameManager.PLAYER_START (480, 480)`, world-centre dungeon, `world_w-80, 20` shop, `world_w-60, world_h-60` rest) so the routes land on the real landmarks. New class constants `TILE_PX = 24`, `PLAYER_START_PX = (480, 480)`, `PATH_CHARS = frozenset("p=|+ro")`.
- **Wired into map loading** — `load_map_from_data` now runs `carve_landmark_paths` right after `declutter_flowers`, so the routes appear regardless of what's on disk (old saved maps and future edits both get routed).
- **Full-map guard** — carving only runs on a full-size overworld: if the map is too small to even contain the spawn point (all landmarks would collapse onto one tile), the grid is returned unchanged (still normalised to a rectangle). This keeps tiny/degenerate maps — and the single-cell probe maps used in tests — from sprouting spurious paths.
- **Baked into the source map** — regenerated `assets/map.txt` through the same declutter+carve pipeline so the bundled 32×24 map ships with the routes already drawn (0 flower tiles remain).
- **Tests** — added `tests/test_structured_paths.py` (14 tests, headless via the `Code.gfx` shim): landmark positions + in-bounds/determinism, route shape preserved, every landmark reached by a path, hub-to-spawn connectivity walked end-to-end, intersections marked, off-route grass untouched, determinism, empty/ragged input handling, and two end-to-end `load_map_from_data` checks proving path tiles render and the spawn tile lands on a route.
- **Verification**: `py_compile` clean; Ruff clean on the new helper + test (the two pre-existing `tile_map.py` findings — unused `random` import, `B007` loop var — predate this change and remain tracked in the ToDo); full suite **213 → 227** green (6 skipped when arcade/display absent). Adjusted one v2.9.0 world-aesthetics test comment to note the small probe line is unaffected by the full-map guard.
- **Result**: roadmap **P5 #31 (Structured Paths) is done.**
...
- **Key finding / real fix** — the old strings leaned on astral-plane emoji (🏆 / 🎉 / ⭐, all code points > U+FFFF), which the `Code.gfx` text renderer **strips** (astral-plane glyphs SIGSEGV pyglet's CoreText path, so the shim drops them). That means the advertised "trophy" never actually rendered on-screen under the Arcade backend — the banner is now built from **Basic-Multilingual-Plane** glyphs that survive the renderer: `★` (U+2605), `✦` (U+2726) and `═` (U+2550).
- **New testable helper** — added a pure, module-level `build_victory_messages(is_boss, xp_gained, credits_gained, leveled_up)` returning `(text, color)` lines (deterministic, no combat state / GPU needed). Boss defeats get a five-line banner: a `★ ═════ ★` border framing `★ 🏆 BOSS DEFEATED! 🏆 ★` (GOLD), `Level Complete!` (YELLOW), the `+XP  +Credits` reward (GREEN), and a matching bottom border; ordinary wins get `✦ Victory! ✦` + the reward; a level-up appends a highlighted `★ LEVEL UP! ★` line in either case. The 🏆 emoji is deliberately kept in the boss line so it still displays under the real-pygame backend / in logs (the shim strips it but the ★ stars remain).
- **Wiring** — `handle_victory` now computes an `is_boss` flag once (drives `complete_level_after_boss()`), keeps the existing level-up sound, and logs each banner line separately so the combat log paints the banner across multiple rows. Removed the old single-line `victory_msg` string-building.
- **Tests** — added `tests/test_victory_messages.py` (10 tests, headless): boss vs. normal banner shape, reward text, level-up line (boss + normal), colour coding, determinism, the combat-log ≤60-char truncation width, and a guard that every on-screen glyph is BMP (≤ U+FFFF) so the decorations can't be silently stripped.
- **Verification**: `py_compile` clean; Ruff clean on the new helper + test (the lone remaining Ruff finding in the file is the pre-existing unused `CharacterManager` import already tracked in the ToDo); full suite **194 → 204** green (6 skipped when arcade/display absent).
- **Result**: roadmap **P5 #29 (special victory messages) is done.**
...
- **pygame → Arcade GUI migration** (MAJOR, v2.x) — completed 07/10/2026; final implementation commit `053d07b`, released as **v2.2.5**. Added `Code/version.py` (central `__version__`), `Code/gfx.py` (pygame→Arcade compat shim with automatic y-axis flip), and `arcade_app.py` (new `arcade.Window` entry point driving the existing state machine at 15 Hz via translated key/text events). The Arcade backend is now the default and the pygame dependency is retired; the detailed implementation history remains below.
...
- **Ruff cleanup + blocking CI lint gate** (CHANGELOG ToDo, deferred from v2.1.2) — done 07/11/2026 on `feature/ruff-cleanup`, released as **v3.2.0**. Cleared every remaining Ruff finding (F401 unused imports, F541 placeholder-less f-strings, B007 unused loop vars, F841 unused locals, one F811 dead redefinition of `CombatManager.calculate_damage`) across `main.py` + `Code/*.py`, all behaviour-preserving dead-code removal. Removed `continue-on-error` from the `Lint (ruff)` step in `.github/workflows/ci.yml` so lint is now a hard gate, and added `server_app.py` to its `py_compile` step. Verified: `ruff check .` clean, `py_compile` clean, full suite **348/348** green (6 skipped when arcade/display absent). This empties the CHANGELOG ToDo — with P0–P6 already complete, there are no tracked work items left. See CHANGELOG.md.
- **Add account creation** (P6 #41) — done 07/11/2026 on `feature/account-creation`, released as **v3.1.0**. Accounts group one or more characters under a single login so a multiplayer player identifies themselves once, then picks which of *their* characters to bring online — while single-player is completely untouched (nothing in the single-player import path references any of this; single-player still discovers characters by scanning `Characters/`). New pure, dependency-free module `Code/account_system.py` (standard library only — `hashlib`/`hmac`/`json`/`os`/`re`/`time`, no pygame/arcade/display): an `AccountManager` doing CRUD over one JSON file per account under an `Accounts/` dir, with case-insensitive filesystem-safe usernames (3–24 chars, `[A-Za-z0-9_.-]`), a minimum-length password stored **only** as a per-account random-salted PBKDF2-HMAC-SHA256 hash (200k iterations, never plaintext), constant-time `verify_password`, character membership (`add_character`/`remove_character`/`list_characters`, idempotent + case-insensitive, capped at 32, and it only indexes existing `Characters/*.json` saves — never duplicates or deletes them), a secret-free `public_view` for anything surfaced to the game/network, and an `authenticate_or_create` convenience for a single multiplayer "Account" prompt. Wired opt-in into the existing multiplayer layer: `Code/multiplayer.py` `MultiplayerSession` gained an optional `account` (public view) field, and `session_from_env()`/new `account_from_env()` log the player in from `MEGITECH_ACCOUNT`(+`_PASSWORD`, optional `MEGITECH_ACCOUNTS_DIR`) — creating the account on first use — and attach the public view; a bad/absent account degrades gracefully to guest play and the display name still falls back explicit-name → account username → "Adventurer". Accountless (guest) multiplayer and single-player both unchanged. Added `Accounts/.gitkeep`. +35 tests (`tests/test_account_system.py`, headless, standard-library only): username/password validation, hashing/verify round-trips + salt independence + corrupt-record handling, full `AccountManager` CRUD + character membership + caps + missing-account errors, `public_view` secret-stripping, and the `MultiplayerSession` env-gating/account attachment. `py_compile` clean; full suite **313 → 348** green (6 skipped when arcade/display absent). **Closes the entire P6 backlog.** See CHANGELOG.md.
- **Multiplayer — client/server networking foundation** (P6 #40) — done 07/11/2026 on `feature/multiplayer-server-client`, released as **v3.0.1**. Added the server component and a client networking layer so multiple players can share a world, with single-player completely untouched (purely additive; nothing in the single-player import path references any of it, and the default `python main.py` never opens a socket). Standard-library only (`socket`/`threading`/`socketserver`/`json`/`struct`) so the server runs headless and the code tests without a display. New modules: `Code/network_protocol.py` (message vocabulary + length-prefixed JSON framing with a `FrameDecoder` stream-reassembly state machine; `PROTOCOL_VERSION=1`, `DEFAULT_PORT=50007`, 8 MiB frame cap), `Code/game_server.py` (`GameServer`, a threaded presence/relay server assigning player ids and broadcasting join/state/leave/chat; relay-only, not a physics authority — combat/world logic stays client-side), `Code/network_client.py` (`NetworkClient` with a background receive thread + thread-safe mirror of other players' `{name,state}`), `Code/multiplayer.py` (opt-in `MultiplayerSession` + env-gated `session_from_env()` reading `MEGITECH_MULTIPLAYER`/`MEGITECH_SERVER`/`MEGITECH_PLAYER_NAME`; failed connects degrade gracefully to single-player), and `server_app.py` (standalone `python server_app.py` launcher, the server counterpart to `arcade_app.py`). +26 tests (`tests/test_network.py`, headless loopback sockets): protocol round-trips/rejections, frame reassembly, server lifecycle, and full client↔server integration (join, state sync, leave, chat, ping/pong, session end-to-end). `py_compile` + Ruff clean on all new code; suite 287→**313** green. **Remaining P6:** #41 account creation; in-game rendering of remote players on the board is the natural next step on top of this transport. See CHANGELOG.md.
- **Add helper pets** (P5 #32) — done 07/11/2026 on `feature/helper-pets`, released as **v2.12.0**. Companion creatures that assist in combat, acquired two ways: a random drop for defeating a boss, or bought from the shop. Added a new pure, display-free module `Code/pet_system.py` — a `PET_CATALOG` of five pets (Dire Wolf, Healing Sprite, Ember Drake, Phoenix, and the boss-only Void Shade) each with an assist behaviour (`attack` damages the enemy, `heal` restores the player, `leech` does both), a level-scaled `compute_pet_assist(pet, level)` (`power + level//2`, deterministic so combat is predictable & testable), shop/boss eligibility helpers, a seeded `roll_boss_pet_drop` (35% chance, excludes already-owned/level-locked pets), and a `PetManager` that reads/writes ownership on the save (`character_data["Pets"]` list + `Active_Pet` id) with a full `buy_pet` flow (affordability / level-lock / already-owned / not-for-sale checks). Wired in: the active pet assists every combat round via a new `EnhancedCombatManager.pet_assist_turn()` (called right before the enemy acts, so a finishing pet strike still resolves to victory); boss victories roll a pet drop in `EnhancedCombatIntegration.award_boss_pet`; the shop lists buyable pets (`item_type="pet"`, sourced from the catalog) and purchases them through `PetManager`. Persistence: bumped `SAVE_SCHEMA_VERSION` 1→2 and taught `save_migration` to backfill/validate `Pets`/`Active_Pet`, and added them to `create_sample_character`. +45 tests (`tests/test_pet_system.py`, headless via the `Code.gfx` shim): catalog integrity, eligibility, assist math, seeded boss-drop, PetManager ownership/purchase, migration backfill, store integration, and combat assist application; updated one v2.3.0 migration fixture for the new container field. `py_compile` clean, new code Ruff-clean; suite 242→**287** green. See CHANGELOG.md.
- **Add more spells** (P5 #33) — done 07/11/2026 on `feature/more-spells`, released as **v2.11.0**. Every aspect now has five spells instead of three: added a **master** tier (unlocks at level 8) and an **ultimate** tier (level 12) to all six aspects — Fire Meteor/Supernova, Water Absolute Zero/Tsunami, Dream Tempest/Cataclysm, Earth Tectonic Shift/World Ender, Life Divine Restoration/Resurrection, Void Void Rift/Oblivion — each with an aspect-appropriate status effect and a mana cost above every earlier tier. Replaced the hard-coded `if level >= 1/3/5` ladder in `SpellManager.get_spells_for_aspect` with a data-driven `SPELL_UNLOCK_LEVELS = (1, 3, 5, 8, 12)` zip (in both `Code/enhanced_combat_system.py` and legacy `Code/combat_system.py`), so the unlocked list grows with level and adapts to any tier count. Made the enhanced combat spell menu's `row_pitch` adaptive (default 45px, floor 34px) so a full five-spell list no longer overruns the combat log (top y=455). +15 tests (`tests/test_more_spells.py`, headless via the `gfx` shim): tier counts, unlock thresholds, ordering/fallbacks, new-tier pricing, legacy↔enhanced parity, and the layout math; `py_compile` clean, new code Ruff-clean; suite 227→**242** green. See CHANGELOG.md.
- **Structured Paths** (P5 #31) — done 07/11/2026 on `feature/structured-paths`, released as **v2.10.0**. The overworld's key interactables (boss dungeon at world centre, shop top-right, rest area bottom-right) had no ground routes leading to them, so the world read as a trackless grass field. Added `EnhancedTileMap.carve_landmark_paths(lines)` (`Code/tile_map.py`) — a pure, deterministic classmethod that carves L-shaped (Manhattan) dirt-path routes from the central hub out to the player spawn, shop, and rest area, marking crossings as intersections (`+`). Added `EnhancedTileMap.landmark_tiles(cols, rows)` (locates hub/spawn/shop/rest from grid size in the same pixel terms the game places them — `PLAYER_START (480,480)`, world-centre dungeon, `world_w-80,20` shop, `world_w-60,world_h-60` rest) plus constants `TILE_PX`/`PLAYER_START_PX`/`PATH_CHARS`. Wired into `load_map_from_data` right after `declutter_flowers`; a full-map guard skips routing on maps too small to hold the spawn point (so tiny probe/test maps aren't affected). Regenerated `assets/map.txt` through the same pipeline so the bundled map ships with routes drawn. +14 tests (`tests/test_structured_paths.py`, headless via the `gfx` shim); `py_compile` + Ruff clean on new code; suite 213→**227** green. See CHANGELOG.md.
- **World Aesthetics — cleaner grass terrain** (P5 #30) — done 07/11/2026 on `feature/world-aesthetics`, released as **v2.9.0**. Removed the decorative flower clutter that littered the overworld and replaced it with clean, natural grass. **Root cause**: `assets/map.txt` sprinkled 40 flower tiles (`F`/`f` patches) across the grass. Added `EnhancedTileMap.declutter_flowers(line, row)` (`Code/tile_map.py`) — a pure, deterministic classmethod that swaps every flower marker (`FLOWER_CHARS = "FfRr"`) for a grass variant chosen by `(row+col) % 3` from `GRASS_VARIANTS = ("G","g","d")`, so the ground stays varied (not one flat block) but flower-free; non-flower tiles (trees/paths/water) pass through untouched. Wired it into `load_map_from_data` so no flower tile survives to the rendered tile map regardless of the on-disk map; rewrote `assets/map.txt` with the same mapping (0 flowers remain); and removed the flower-circle detail from the procedural fallback tileset (tiles 12/13 → plain grass). +9 tests (`tests/test_world_aesthetics.py`, headless via the `gfx` shim); `py_compile` clean, new code Ruff-clean; suite 204→213 green. See CHANGELOG.md.
