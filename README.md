...
**2026-07-11 (v3.2.0)**: **Ruff cleanup + blocking CI lint gate** (CHANGELOG ToDo, deferred from v2.1.2). Cleared every remaining Ruff finding across `main.py` + `Code/*.py` — unused imports (F401), placeholder-less f-strings (F541), unused loop control variables (B007), unused local variables (F841), and one dead redefinition of `CombatManager.calculate_damage` (F811) — all behaviour-preserving dead-code removal. Flipped the CI `Lint (ruff)` step to **blocking** (removed `continue-on-error` in `.github/workflows/ci.yml`) so any new finding now fails the build, and added `server_app.py` to its `py_compile` step. `ruff check .` is clean, `py_compile` clean, full suite **348/348** green (6 skipped when arcade/display absent). With the ROADMAP P0–P6 backlog already complete, this empties the CHANGELOG ToDo — no tracked work items remain. See CHANGELOG.md.

**2026-07-11 (v3.1.0)**: **Account creation** (roadmap P6 #41). Multiplayer players can now create an account and link their characters to it, logging in once and then choosing which of *their* characters to bring online — while **single-player is completely unchanged** (purely additive; nothing in the single-player import path references the account layer, and single-player still discovers characters by scanning `Characters/`). New **standard-library-only** module `Code/account_system.py` (`hashlib`/`hmac`/`json`/`os`/`re`/`time` — no pygame/arcade, unit-tests headlessly): `AccountManager` does CRUD over one JSON file per account under `Accounts/`, with case-insensitive filesystem-safe usernames, passwords stored **only** as a random-salted PBKDF2-HMAC-SHA256 hash (never plaintext) verified in constant time, character membership that only *indexes* existing `Characters/*.json` saves (never duplicates or deletes them, capped at 32), a secret-free `public_view`, and an `authenticate_or_create` convenience for a single multiplayer login prompt. Wired opt-in into `Code/multiplayer.py`: `session_from_env()`/`account_from_env()` log the player in from `MEGITECH_ACCOUNT`(+`_PASSWORD`) — creating the account on first use — and attach the public view to the session; a bad/absent account degrades gracefully to guest play. +35 headless tests (`tests/test_account_system.py`); full suite **313 → 348** green. Closes the P6 backlog. See CHANGELOG.md.
...
**2026-07-11 (v2.11.0)**: **More spells** (roadmap P5 #33). Every magic aspect now has five spells instead of three. Two new tiers unlock as you level: a **master** spell at level 8 and an **ultimate** spell at level 12 — Fire gains Meteor/Supernova, Water Absolute Zero/Tsunami, Dream Tempest/Cataclysm, Earth Tectonic Shift/World Ender, Life Divine Restoration/Resurrection, and Void Void Rift/Oblivion, each with an aspect-appropriate status effect and a mana cost above every earlier tier. The old hard-coded 3-spell unlock ladder was replaced with a data-driven `SPELL_UNLOCK_LEVELS = (1, 3, 5, 8, 12)` so the available list grows with level, and the combat spell menu now uses an adaptive row pitch so the longer list never overruns the combat log. +15 tests (`tests/test_more_spells.py`); full suite **227 → 242** green. See CHANGELOG.md.
...
│   ├── multiplayer.py          # Opt-in high-level multiplayer session (v3.0.1, +accounts v3.1.0)
│   ├── account_system.py       # Player accounts grouping characters for multiplayer (v3.1.0)
│   └── debug.py                # Gated debug output (MEGITECH_DEBUG)
├── tests/                      # Headless unittest suite
├── Accounts/                   # Player account files (gitignored, runtime — v3.1.0)
